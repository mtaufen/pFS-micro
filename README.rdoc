...
*Note:* This will only write over the data in the file, it will not replace it. 
For example, if you had a file called 'text.txt' and it contained the string `awesomestring`,
and you performed writeOver on it with a data_string of 'str', 'text.txt' would then contain *str*somestring
